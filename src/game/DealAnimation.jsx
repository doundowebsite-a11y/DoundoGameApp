...
export default function DealAnimation({
  ...
  useEffect(() => {
    ...
    // bending them upward, cards interleaving sequentially to the middle.
    // Camera angle: slightly above, slightly to the side = 3/4 view.

    const buildRiffleOut = (spread, arch, rotZMax, rotXVal) =>
      cards.map((c, i) => {
        const isLeft = i % 2 === 0; // split by even/odd for interleaving
        const halfI  = Math.floor(i / 2);
        const halfN  = N_DEAL / 2;
        
        const archY = -Math.sin((halfI / (halfN - 1 || 1)) * Math.PI) * arch;
        const xOffset = isLeft ? -spread + halfI * 3 : spread - halfI * 3;
        const zTilt = isLeft ? -rotZMax : rotZMax;

        return Animated.parallel([
          Animated.timing(c.x, { toValue: xOffset, duration: 250, easing: Easing.out(Easing.ease), useNativeDriver: true }),
          Animated.timing(c.y, { toValue: archY, duration: 250, easing: Easing.out(Easing.ease), useNativeDriver: true }),
          Animated.timing(c.rotZ, { toValue: zTilt, duration: 250, useNativeDriver: true }),
          Animated.timing(c.rotX, { toValue: rotXVal, duration: 250, useNativeDriver: true }),
        ]);
      });

    const buildRiffleIn = () =>
      cards.map((c, i) => {
        // Interleave stagger: cards fall to the center one by one in array order (which is now alternating left/right)
        return Animated.parallel([
          Animated.spring(c.x,    { toValue: 0, friction: 6, tension: 70, delay: i * 30, useNativeDriver: true }),
          Animated.spring(c.y,    { toValue: 0, friction: 6, tension: 70, delay: i * 30, useNativeDriver: true }),
          Animated.spring(c.rotZ, { toValue: 0, friction: 6, tension: 70, delay: i * 30, useNativeDriver: true }),
          Animated.spring(c.rotX, { toValue: 0, friction: 6, tension: 70, delay: i * 30, useNativeDriver: true }),
        ]);
      });

    const riffleOut1  = buildRiffleOut(55, 45, 25, 20);
    const riffleIn1   = buildRiffleIn();
    const riffleOut2  = buildRiffleOut(45, 35, 18, 15);
