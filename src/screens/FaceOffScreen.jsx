...
/*
  FaceOffScreen
  ─────────────
  Shows both players' full cards with cream (#f2e3c6) background.
  ONLY ONE BUTTON: NEXT → goes to WinScreen or LoseScreen
...
const FaceOffScreen = ({ navigation, route }) => {
  ...
  const isTie = overlay.winner === 'tie' || (!playerWon && overlay.winner !== 'ai');
  ...
  const handleNext = () => {
    ...
    const params = {
      points: finalPoints, totalCards: 42, time: '05:00',
    ...
    if (wonGame && lostGame) {
      navigation.replace('DrawScreen', params);
    } else if (wonGame) {
      navigation.replace('WinScreen', params);
    } else if (lostGame) {
      navigation.replace('LoseScreen', params);
    } else {
      // Round over, match continues.
      // Use overlay.winner to decide this player's result screen —
      // multiplayerReducer already flipped winner correctly per player.
      if (playerWon) {
        navigation.replace('WinScreen', { ...params, isRoundOnly: true });
      } else if (overlay.winner === 'ai') {
        navigation.replace('LoseScreen', { ...params, isRoundOnly: true });
      } else {
        navigation.replace('DrawScreen', { ...params, isRoundOnly: true });
      }
    }
  };
  ...
  const pSyms = overlay.pSyms ?? [];
  const aSyms = overlay.aSyms ?? [];
...
const styles = StyleSheet.create({
  ...
  // Each card has uniform cream background
  faceCard: {
    borderRadius: 8,
    borderWidth: 1.5,
  ...
  faceCardInner: {
    flex: 1,
    // Uniform cream background same as hand cards
