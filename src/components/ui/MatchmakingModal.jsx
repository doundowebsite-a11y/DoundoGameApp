...
export default function MatchmakingModal({ visible, status, elapsed, opponent, onCancel }) {
      ...
      <View style={s.card}>
        {(status === 'found' || status === 'starting') && opponent ? (
          ...
          <>
            ...
            <Text style={s.subtitle}>Searching for an opponent…</Text>
            <SearchRing />
            <Text style={s.timer}>{fmt(elapsed)}</Text>
            <Text style={s.tip}>{tipText}</Text>
